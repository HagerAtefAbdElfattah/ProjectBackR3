...
// ---taking the data by post-----------
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
      try{
        ...
        $sql = "INSERT INTO `users`(`name`, `userName`, `email`, `password`, `image`, `active`) VALUES (?,?,?,?,?,?)";
        $name =$_POST["name"];
		$userName =$_POST["user-name"];
        $email =$_POST["email"];
        $password =password_hash($_POST["password"], PASSWORD_DEFAULT);
		//-----define the user is active or not---------
		...
		//----- insert image---------------
		 include_once("includes/userImage.php");
        $stmt = $conn->prepare($sql);
        $stmt->execute([$name, $userName, $email, $password, $image_name, $active]);
